...
const pages = {feed:'Находки',train:'Твой вкус',outfits:'Образы',saved:'Сохранённое',digest:'На сегодня',sources:'Магазины',settings:'Настройки'};
...
function settingsPage() {
  ...
  $('#settingsForm').addEventListener('submit', async e=>{
    ...
    for(const k of ['daily_enabled','strict_sizes','hide_unknown_prices','browser_fallback'])updated[k]=f.has(k);
    for(const k of ['daily_time','gender','season','outfit_mode'])updated[k]=f.get(k);
    for(const k of ['digest_count','page_budget'])updated[k]=Number(f.get(k));
